...
function BEMHTML(options) {
  ...
  this._entitiesSet = typeof options.entitiesSet !== 'undefined' && new Set();
}
...
BEMHTML.prototype.render = function(context, entity, tag, js, bem, cls, mix,
  ...
  if (isBEM) {
    ...
    if (this._entitiesSet)
      this._entitiesSet.add(entity.jsClass);
...
BEMHTML.prototype.renderMix = function(entity, mix, jsParams, addJSInitClass) {
  ...
  for (var i = 0; i < mix.length; i++) {
    ...
    if (hasItem) {
      ...
      if (this._entitiesSet)
        this._entitiesSet.add(className);
    }
...
BEMXJST.prototype.locals = Tree.methods
    .concat('local', 'applyCtx', 'applyNext', 'apply', 'entitiesSet');
...
BEMXJST.prototype.compile = function(code) {
  ...
  function entitiesSet() {
    var entities = self._entitiesSet;

    self._entitiesSet = new Set();
